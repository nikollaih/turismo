...
if (!function_exists('trx_addons_build_slider_layout')) {
	function trx_addons_build_slider_layout($args=array(), $images=array()) {
		$args = array_merge(array(
			...
			'pagination_pos' => 'bottom',	// bottom | left | right - position of the pagination
		...
		if ($args['pagination_type']=='progress') {
			if ($args['direction']=='vertical' && $args['pagination_pos']=='bottom')
				$args['pagination_pos'] = 'left';
			if ($args['direction']=='horizontal' && in_array($args['pagination_pos'], array('left', 'right')))
		...
		if (is_array($images) && count($images) > 0) {
			...
			$pagination_inside = trx_addons_is_on($args['pagination']) 
								&& ($args['pagination_type']=='progress' 
									|| ($args['pagination_type']=='bullets' && in_array($args['pagination_pos'], array('left', 'right')))
									);
			
			$output .= '</div>'
				. ($args['style']=='modern' || $args['titles']=='outside' 
						? (($pagination_inside
				...
				. (trx_addons_is_on($args['controls']) 
						? '<div class="slider_controls_wrap">'
							...
							. '</div>' 
						: '')
				. (trx_addons_is_on($args['pagination']) && ($args['style']!='modern' && $args['titles']!='outside' || !$pagination_inside)
						? '<div class="slider_pagination_wrap swiper-pagination"></div>' 
						: '')
